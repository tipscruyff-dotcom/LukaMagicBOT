...
async def create_one_time_invite_link(
    ...
    for group_id in target_groups:
        ...
        while attempt < attempts:
            attempt += 1
            try:
                logger.info(f"Trying to create invite for group: {group_id} (attempt {attempt}/{attempts})")
                ...
                error_msg = str(e)
                last_err_text = error_msg
                # Handle flood wait / rate limits
                lower = error_msg.lower()
                if "too many requests" in lower or "retry after" in lower:
                    # Extract retry seconds if present
                    import re as _re
                    m = _re.search(r"retry after (\d+)", lower)
                    wait_s = int(m.group(1)) if m else (1 + attempt)
                    logger.warning("Rate limited creating invite for group %s; sleeping %ss", group_id, wait_s)
                    await asyncio.sleep(wait_s)
                    continue
                # Permission problems won't be fixed by retrying
                if "not enough rights" in lower or "forbidden" in lower or "need administrator rights" in lower:
                    logger.error("Bot lacks admin permissions in group %s: %s", group_id, error_msg)
                    break
                # Generic transient error: small backoff then retry
                if attempt < attempts:
                    backoff = 0.8 * attempt
                    logger.warning("Invite creation transient error for group %s: %s (retrying in %.1fs)", group_id, error_msg, backoff)
                    await asyncio.sleep(backoff)
                else:
                    break
        if group_id not in success_groups:
            failed_groups[group_id] = last_err_text or "unknown_error"
    ...
    # If nothing worked, try safe fallbacks
    if not invite_links and VIP_INVITE_LINK:
        logger.warning("Using configured static VIP link as fallback")
...
async def stripe_webhook(request: Request):
    ...
    # Extract client info for Meta CAPI (optional)
    ...
    with SessionLocal() as db:
        logger.info("DB URL runtime: %s", db_path_info(db))
        try:
            success = await process_stripe_webhook_event(db, event, client_ip=client_ip, user_agent=user_agent)
            if success:
                logger.info(
...
@app.get("/admin/migrations/plan-type", response_class=HTMLResponse)
async def admin_plan_type_migration_form(request: Request):
    _require_admin(request)

    body = """
    <h1>Plan Type Migration</h1>
    <p>Executa a migracao para padronizar o campo <code>plan_type</code> e evitar valores nulos.</p>
    <p>Use apenas se voce tiver certeza de que o banco PostgreSQL do Railway esta acessivel.</p>
    <form method="post" action="/admin/migrations/plan-type">
        <button type="submit" style="padding: 10px 18px;">Executar migracao agora</button>
    </form>
    <p><small>Comandos executados:</small></p>
    <pre>UPDATE subscriptions SET plan_type = 'unknown' WHERE plan_type IS NULL;
ALTER TABLE subscriptions ALTER COLUMN plan_type SET DEFAULT 'unknown';
ALTER TABLE subscriptions ALTER COLUMN plan_type SET NOT NULL;</pre>
    <p><a href="/admin/subscriptions">Voltar para Subscriptions</a></p>
    """
    return HTMLResponse(_html_page("Plan Type Migration", body))


@app.post("/admin/migrations/plan-type", response_class=HTMLResponse)
async def admin_run_plan_type_migration(request: Request):
    _require_admin(request)
        ...
        try:
            with db.begin():
                for stmt in statements:
                    ...
                    pending_results.append({"statement": stmt, "rowcount": rowcount})
        except Exception as e:
            migration_error = f"{current_stmt or 'migracao'}: {e}"
            logger.error("Plan type migration failed: %s", e, exc_info=True)
            try:
                db.rollback()
            except Exception:
                pass

        results = [{"statement": r["statement"], "rowcount": r["rowcount"], "ok": migration_error is None} for r in pending_results]

        items = ""
        for r in results:
            affected = ""
            if r["rowcount"] not in (None, -1):
                affected = f" - linhas afetadas: {r['rowcount']}"
            items += f"<li>{'OK' if r['ok'] else 'ERRO'} <code>{html.escape(r['statement'])}</code>{affected}</li>"

        if migration_error:
            items += f"<li>ERRO <strong>Detalhe:</strong> {html.escape(migration_error)}</li>"

        body = f"""<h1>Plan Type Migration</h1>
<p>{'Migracao executada com sucesso.' if not migration_error else 'Falha ao executar a migracao.'}</p>
<ul>{items}</ul>
<p><a href="/admin/migrations/plan-type">Voltar</a></p>
<p><a href="/admin/subscriptions">Voltar para Subscriptions</a></p>"""
