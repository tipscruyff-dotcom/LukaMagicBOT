...
async def process_stripe_webhook_event(
    ...
    try:
        if event_type == "checkout.session.completed":
            ...
            # Send Meta Conversions API Purchase event (non-blocking, non-fatal)
            try:
                ...
                # Extract email from invoice
                customer_email = invoice.get("customer_email")
                if not customer_email:
                    customer_details = invoice.get("customer_details") or {}
                    customer_email = customer_details.get("email")
                
                # Extract event ID for deduplication (prefer payment_intent, fallback to invoice ID)
                payment_intent = invoice.get("payment_intent")
                capi_event_id = payment_intent or invoice.get("id") or event_id
                
                # Send to Meta CAPI
                if customer_email:
                    ok, err = await send_purchase_event(
                        ...
                        source_url=os.getenv("SALES_SITE_URL", "https://lukamagiceurope.com")
                    )
                    if not ok:
                        logger.warning(f"Meta CAPI Purchase not sent: {err}")
                    else:
                        logger.info(f"Meta CAPI Purchase event sent for invoice {invoice.get('id')}")
                else:
                    logger.debug("Skipping Meta CAPI: no customer email in invoice")
            except ImportError:
                # meta_capi module not available or dependencies missing - silent skip
                logger.debug("Meta CAPI module not available, skipping")
            except Exception as e:
                # Never break the webhook - log and continue
                logger.warning(f"Meta CAPI integration error (non-fatal): {e}", exc_info=True)
